    ...
    <script>
        ...
        function displayResults(data) {
            ...
            document.getElementById('resultsSummary').textContent = 
                `${data.sections.length} sections identified`;
        ...
        function generateDocument() {
            ...
            const tocLines = selectedSections.map((section, index) => 
                `${index + 1}. ${section.title}${' '.repeat(50 - section.title.length)}Page ${section.page}`
            );
...
def analyze():
    try:
        ...
        # Analyze the document using the exact categories the user specified
